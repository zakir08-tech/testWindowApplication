...
public class LaunchScreen extends Application {
    ...
    private Stage testManagerStage = null;
    private Stage testRunnerStage = null;
    ...
    public void start(Stage primaryStage) {
        try {
            ...
            Text subtitle = new Text("Window Testing Framework");
            ...
            createEditButton.setOnAction(e -> {
                try {
                    if (testManagerStage == null || !testManagerStage.isShowing()) {
                        testManagerStage = new Stage();
                        TestManagerApp testManagerApp = new TestManagerApp();
                        testManagerApp.start(testManagerStage);
                        openStages.add(testManagerStage);
                    } else {
                        testManagerStage.requestFocus(); // Bring existing window to focus
                    }
                } catch (Exception ex) {
            ...
            runTestButton.setOnAction(e -> {
                try {
                    if (testRunnerStage == null || !testRunnerStage.isShowing()) {
                        testRunnerStage = new Stage();
                        TestRunner testRunner = new TestRunner();
                        testRunner.start(testRunnerStage);
                        openStages.add(testRunnerStage);
                    } else {
                        testRunnerStage.requestFocus(); // Bring existing window to focus
                    }
                } catch (Exception ex) {
            ...
            primaryStage.setTitle("Phantom: Window Testing Framework");
