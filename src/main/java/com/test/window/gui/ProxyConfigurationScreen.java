...
/**
 * Proxy Configuration Manager - Saves in nested JSON format to proxy.json
 */
public class ProxyConfigurationScreen extends Application {
    ...
    private boolean isValidProxyName(String name, Set<String> nameSet, String currentId) {
    ...
    private void stopEditing() {
        if (table.getEditingCell() != null) {
            ...
            if (cell instanceof CustomTextFieldTableCell customCell) {
                if (customCell.getGraphic() instanceof javafx.scene.control.TextField tf) {
                    String text = tf.getText();
                    if (customCell.columnIndex == 0 && !text.isEmpty()) {
                        if (isValidProxyName(text, customCell.nameSet, customCell.originalValue)) {
    ...
    private TableCell<String[], String> getTableCellAt(int rowIndex, int columnIndex) {
        for (Node node : table.lookupAll(".table-cell")) {
            if (node instanceof TableCell<?, ?> cell
                    ...
                    && cell.getTableRow().getIndex() == rowIndex
                    && cell.getTableColumn() == table.getColumns().get(columnIndex)) {
    ...
    private void reloadProxyJson() {
        try {
            File file = new File("proxy.json");
            if (!file.exists()) {
            ...
            for (Map.Entry<String, Object> entry : root.entrySet()) {
                ...
                if (config instanceof Map<?, ?> map) {
                    row[1] = getString(map, "server");
                    row[2] = getString(map, "port");
                    row[3] = getString(map, "username");
                    row[4] = getString(map, "password");
                }
            ...
            System.err.println("Failed to load proxy.json: " + ex.getMessage());
    ...
    public void start(Stage primaryStage) {
        ...
        saveButton.setOnAction(e -> {
            ...
            Map<String, Map<String, String>> proxyConfigs = new LinkedHashMap<>();

            for (String[] row : table.getItems()) {
                String name = row[0] != null ? row[0].trim() : "";
                if (name.isEmpty()) continue;

                String server = row[1] != null ? row[1].trim() : "";
                String port = row[2] != null ? row[2].trim() : "";
                String username = row[3] != null ? row[3].trim() : "";
                String password = row[4] != null ? row[4].trim() : "";

                Map<String, String> profile = new LinkedHashMap<>();
                profile.put("server", server);
                profile.put("port", port);
                profile.put("username", username);
                profile.put("password", password);

                proxyConfigs.put(name, profile);
            }
            ...
            try {
                ...
                try (FileOutputStream fos = new FileOutputStream(file)) {
                    objectMapper.writeValue(fos, proxyConfigs);
                }
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setContentText("Proxy configurations saved successfully!");
                alert.showAndWait();
                hasUnsavedChanges = false;
                // Optional: notify other parts of app
                // UIComponentsManager.getInstance().loadProxyProfilesIntoComboBox();
        ...
        primaryStage.focusedProperty().addListener((obs, oldV, newV) -> {
            if (newV && !hasUnsavedChanges) reloadProxyJson();
        });
        ...
        primaryStage.setTitle("Proxy Configuration Manager");
        primaryStage.show();
    ...
    private TableView<String[]> createTable() {
        ...
        Label placeholderLabel = new Label("No proxy configurations defined");
        ...
        for (int i = 0; i < COLUMN_NAMES.length; i++) {
            ...
            column.setPrefWidth(i == 2 ? 100 : 200); // Port column narrower
            column.setStyle("-fx-text-fill: white; -fx-alignment: CENTER-LEFT;");

            column.setSortable(i == 0); // Only Proxy Name is sortable
    ...
    private class CustomTextFieldTableCell extends TextFieldTableCell<String[], String> {
        ...
        public void startEdit() {
            ...
            if (getGraphic() instanceof javafx.scene.control.TextField tf) {
                if (columnIndex == 0) { // Proxy Name column
                    tf.setTextFormatter(new TextFormatter<>(change -> {
                        String text = change.getControlNewText();
                        if (text.isEmpty()) return change;
                        if (!text.matches("^[a-zA-Z][a-zA-Z0-9_]*$")) {
                            statusLabel.setText("Invalid name: must start with letter, only letters, digits, underscore");
                            return null;
                        }
                        if (!text.equalsIgnoreCase(originalValue) && nameSet.contains(text.toLowerCase())) {
                            statusLabel.setText("Proxy Name already exists!");
                            return null;
                        }
                        statusLabel.setText("");
                        return change;
                    }));
                } else if (columnIndex == 2) { // Port column - numbers only
                    tf.setTextFormatter(new TextFormatter<>(change -> {
                        String text = change.getControlNewText();
                        if (text.isEmpty()) return change;
                        if (!text.matches("\\d*")) return null;
                        if (text.length() > 5) return null; // reasonable port limit
                        return change;
                    }));
                }
        ...
        public void commitEdit(String newValue) {
            if (columnIndex == 0 && !newValue.isEmpty() && !isValidProxyName(newValue, nameSet, originalValue)) {
                return;
            }
